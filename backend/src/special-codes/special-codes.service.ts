...
export class SpecialCodesService {
  ...
  async updateCode(id: string, dto: UpdateSpecialCodeDto) {
    ...
    if (dto.code !== undefined) {
      specialCode.code = this.normalizeCode(dto.code);
    }
  ...
  async getSalesForOwner(ownerUserId: string) {
    ...
    return orders.map((order) => ({
      ...
      buyerName: [order.user?.firstName, order.user?.lastName].filter(Boolean).join(' '),
  ...
  async getAllSales() {
    ...
    return orders
      ...
      .map((order) => ({
        ...
        buyerName: [order.user?.firstName, order.user?.lastName].filter(Boolean).join(' '),
        buyerEmail: order.user?.email || '',
        ownerName: [order.specialCodeOwner?.firstName, order.specialCodeOwner?.lastName].filter(Boolean).join(' '),
        ticketCount: order.ticketCount,
        total: Number(order.total),
        purchasedAt: order.paidAt || order.createdAt,
      }));
