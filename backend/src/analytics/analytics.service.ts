...
export class AnalyticsService {
  ...
  async getSummary(days = 7, eventSlug?: string) {
    ...
    const financialQuery = this.orderRepo
      ...
      .select('COUNT(ord.id)', 'paidOrders')
      .addSelect('COALESCE(SUM(ord.total), 0)', 'totalCharged')
      ...
      .where('ord.status = :paidStatus', { paidStatus: 'paid' })
      .andWhere('COALESCE(ord."paidAt", ord."createdAt") >= :since', { since });
