...
La emisión confirmada genera una sola copia operativa del ticket: si existe correo del comprador, LPTicket recibe la copia oculta habitual; si no existe, `TICKET_ARCHIVE_EMAIL` —con respaldo en `info@lpticket.com`— es el único destinatario. Una entrega solicitada posteriormente por el cliente suprime las copias operativas para evitar duplicados.
