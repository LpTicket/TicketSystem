    ...
    <div className="premium-shell space-y-6 p-6 lg:p-8">
      ...
      <section className="overflow-hidden rounded-2xl border border-[rgba(10,55,90,0.10)] bg-white shadow-sm">
        ...
        <div className="grid grid-cols-1 gap-3 p-4 md:grid-cols-2 xl:grid-cols-4">
          ...
          <div className="rounded-2xl border border-orange-400/45 bg-orange-950/85 p-4 shadow-sm shadow-orange-950/30"><p className="text-xs font-bold uppercase tracking-wide text-orange-200">{lang === 'es' ? 'Ajuste adicional de Klarna' : 'Additional Klarna adjustment'}</p><p className="mt-2 text-2xl font-black text-white">-{money(summary.organizerProcessingAdjustments)}</p><p className="mt-1 text-xs font-semibold text-orange-100">{summary.pendingFeeReconciliations > 0 ? `${summary.pendingFeeReconciliations} ${lang === 'es' ? 'orden(es) esperando el costo real de Stripe' : 'order(s) waiting for the actual Stripe fee'}` : (lang === 'es' ? 'Diferencia sobre la tarifa estándar de tarjeta' : 'Difference above the standard card fee')}</p></div>
          <div className="rounded-2xl border border-orange-400/45 bg-orange-950/85 p-4 shadow-sm shadow-orange-950/30"><p className="text-xs font-bold uppercase tracking-wide text-orange-200">{lang === 'es' ? 'Pagado al organizador' : 'Paid to organizer'}</p><p className="mt-2 text-2xl font-black text-white">{money(summary.organizerPaid)}</p><p className="mt-1 text-xs font-semibold text-orange-100">{detail.organizerPayouts.length} {lang === 'es' ? 'pago(s) registrado(s)' : 'recorded payout(s)'}</p></div>
          <div className="rounded-2xl border border-orange-400/45 bg-orange-950/85 p-4 shadow-sm shadow-orange-950/30"><p className="text-xs font-bold uppercase tracking-wide text-orange-200">{lang === 'es' ? 'Pendiente por pagar' : 'Pending payout'}</p><p className="mt-2 text-2xl font-black text-white">{money(summary.organizerPending)}</p><p className="mt-1 text-xs font-semibold text-orange-100">{lang === 'es' ? 'Saldo que aún debes transferir' : 'Balance still to transfer'}</p></div>
        </div>
