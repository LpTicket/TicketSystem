...
export default function AdminEventFinancialDetailPage() {
  ...
  const openPayoutModal = () => {
    if (!detail || detail.summary.organizerPending <= 0 || detail.summary.pendingFeeReconciliations > 0) return;
  ...
  const cards = [
    { label: lang === 'es' ? 'Ingresos cobrados (con fees)' : 'Gross revenue (with fees)', value: money(summary.grossCharged), note: `${money(summary.lpFees + summary.processingFees)} ${lang === 'es' ? 'en fees cobrados' : 'in fees charged'}`, icon: HiOutlineCurrencyDollar, tone: 'cyan' },
    { label: lang === 'es' ? 'Ingresos por entradas' : 'Ticket revenue', value: money(summary.ticketRevenue), note: lang === 'es' ? 'Precio de entradas, sin fees' : 'Ticket prices, excluding fees', icon: HiOutlineCurrencyDollar, tone: 'sky' },
    { label: lang === 'es' ? 'Comisión LPTicket' : 'LPTicket fees', value: money(lpticketFees), note: lang === 'es' ? 'Cargo sobre el precio base' : 'Markup over base price', icon: HiOutlineCurrencyDollar, tone: 'orange' },
    { label: lang === 'es' ? 'Comisión Stripe' : 'Stripe fees', value: `-${money(estimatedStripeFees)}`, note: lang === 'es' ? 'Estimación estándar por pago' : 'Standard estimate per payment', icon: HiOutlineCurrencyDollar, tone: 'purple' },
    { label: lang === 'es' ? 'Ganancia LPTicket' : 'LPTicket profit', value: money(lpticketProfit), note: lang === 'es' ? 'Comisión menos Stripe' : 'Fees minus Stripe', icon: HiOutlineCurrencyDollar, tone: 'green' },
    { label: lang === 'es' ? 'Órdenes pagadas' : 'Paid orders', value: String(summary.paidOrders), note: `${summary.buyers} ${lang === 'es' ? 'compradores únicos' : 'unique buyers'}`, icon: HiOutlineClipboardList, tone: 'amber' },
    { label: lang === 'es' ? 'Entradas pagadas' : 'Paid tickets', value: String(summary.expectedTickets), note: lang === 'es' ? 'Cantidad confirmada en órdenes' : 'Quantity confirmed in orders', icon: HiOutlineTicket, tone: 'blue' },
    { label: lang === 'es' ? 'Entradas emitidas' : 'Issued tickets', value: String(summary.issuedTickets), note: `${summary.scannedTickets} ${lang === 'es' ? 'escaneadas ·' : 'scanned ·'} ${summary.pendingTickets} ${lang === 'es' ? 'pendientes' : 'pending'}`, icon: HiOutlineCheckCircle, tone: 'teal' },
    { label: lang === 'es' ? 'Bloqueos de mapa' : 'Map locks', value: String(summary.lockedSeats), note: lang === 'es' ? 'No son ventas ni ingresos' : 'Not sales or revenue', icon: HiOutlineUsers, tone: 'rose' },
  ];
  const cardTones: Record<string, { surface: string; marker: string; label: string; value: string; note: string; icon: string }> = {
    cyan: { surface: 'border-cyan-400/30 bg-cyan-500/10', marker: 'financial-tone-cyan', label: 'text-cyan-300', value: 'text-cyan-300', note: 'text-cyan-100/65', icon: 'bg-cyan-500/20 text-cyan-300' },
    sky: { surface: 'border-sky-400/30 bg-sky-500/10', marker: 'financial-tone-sky', label: 'text-sky-300', value: 'text-sky-300', note: 'text-sky-100/65', icon: 'bg-sky-500/20 text-sky-300' },
    orange: { surface: 'border-orange-400/35 bg-orange-500/10', marker: 'financial-tone-orange', label: 'text-orange-300', value: 'text-orange-300', note: 'text-orange-100/65', icon: 'bg-orange-500/20 text-orange-300' },
    purple: { surface: 'border-purple-400/35 bg-purple-500/15', marker: 'financial-tone-purple', label: 'text-purple-300', value: 'text-purple-300', note: 'text-purple-100/65', icon: 'bg-purple-500/20 text-purple-300' },
    green: { surface: 'border-emerald-400/35 bg-emerald-500/15', marker: 'financial-tone-emerald', label: 'text-emerald-300', value: 'text-emerald-300', note: 'text-emerald-100/65', icon: 'bg-emerald-500/20 text-emerald-300' },
    amber: { surface: 'border-amber-400/35 bg-amber-500/15', marker: 'financial-tone-amber', label: 'text-amber-300', value: 'text-amber-300', note: 'text-amber-100/65', icon: 'bg-amber-500/20 text-amber-300' },
    blue: { surface: 'border-blue-400/30 bg-blue-500/10', marker: 'financial-tone-blue', label: 'text-blue-300', value: 'text-blue-300', note: 'text-blue-100/65', icon: 'bg-blue-500/20 text-blue-300' },
    teal: { surface: 'border-teal-400/30 bg-teal-500/10', marker: 'financial-tone-teal', label: 'text-teal-300', value: 'text-teal-300', note: 'text-teal-100/65', icon: 'bg-teal-500/20 text-teal-300' },
    rose: { surface: 'border-rose-400/30 bg-rose-500/10', marker: 'financial-tone-rose', label: 'text-rose-300', value: 'text-rose-300', note: 'text-rose-100/65', icon: 'bg-rose-500/20 text-rose-300' },
  };
  ...
  return (
    <div className="premium-shell space-y-6 p-6 lg:p-8">
      ...
      <section className="overflow-hidden rounded-2xl border border-[rgba(10,55,90,0.10)] bg-white shadow-sm">
        ...
        <div className="grid grid-cols-1 gap-3 p-4 sm:grid-cols-2 xl:grid-cols-3">{cards.map((card) => { const tone = cardTones[card.tone]; return <div key={card.label} className={`rounded-2xl border p-4 ${tone.surface} ${tone.marker}`}><div className="flex items-start justify-between gap-3"><div><p className={`financial-card-label text-xs font-bold uppercase tracking-wide ${tone.label}`}>{card.label}</p><p className={`financial-card-value mt-2 text-2xl font-black ${tone.value}`}>{card.value}</p><p className={`financial-card-note mt-1 text-xs font-semibold ${tone.note}`}>{card.note}</p></div><div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-xl ${tone.icon}`}><card.icon className="h-5 w-5" /></div></div></div>; })}</div>
      </section>
      ...
      <section className="overflow-hidden rounded-2xl border border-[rgba(10,55,90,0.10)] bg-white shadow-sm">
        <div className="flex flex-col gap-4 border-b border-gray-100 bg-gradient-to-r from-[#0A375A] to-[#123f65] px-5 py-4 text-white sm:flex-row sm:items-center sm:justify-between">
          ...
          <button onClick={openPayoutModal} disabled={summary.organizerPending <= 0 || summary.pendingFeeReconciliations > 0} className="rounded-xl bg-[#F97316] px-4 py-2.5 text-sm font-black text-white shadow-sm transition hover:bg-[#EA580C] disabled:cursor-not-allowed disabled:opacity-50">
            {summary.pendingFeeReconciliations > 0
              ? (lang === 'es' ? 'Esperando costo de Stripe' : 'Waiting for Stripe fee')
              : summary.organizerPending > 0
                ? (lang === 'es' ? 'Registrar pago' : 'Record payout')
                : (lang === 'es' ? 'Saldo pagado' : 'Balance paid')}
          </button>
        </div>
        <div className="grid grid-cols-1 gap-3 p-4 md:grid-cols-2 xl:grid-cols-4">
          <div className="financial-tone-sky rounded-2xl border border-sky-400/30 bg-sky-500/10 p-4"><p className="financial-card-label text-xs font-bold uppercase tracking-wide text-sky-300">{lang === 'es' ? 'Monto para organizador' : 'Organizer ticket revenue'}</p><p className="financial-card-value mt-2 text-2xl font-black text-sky-300">{money(summary.ticketRevenue)}</p><p className="financial-card-note mt-1 text-xs font-semibold text-sky-100/65">{lang === 'es' ? 'Ingresos por entradas, sin fees' : 'Ticket revenue, excluding fees'}</p></div>
          <div className="financial-tone-orange rounded-2xl border border-orange-400/35 bg-orange-500/15 p-4"><p className="financial-card-label text-xs font-bold uppercase tracking-wide text-orange-300">{lang === 'es' ? 'Ajuste adicional de Klarna' : 'Additional Klarna adjustment'}</p><p className="financial-card-value mt-2 text-2xl font-black text-orange-300">-{money(summary.organizerProcessingAdjustments)}</p><p className="financial-card-note mt-1 text-xs font-semibold text-orange-100/65">{summary.pendingFeeReconciliations > 0 ? `${summary.pendingFeeReconciliations} ${lang === 'es' ? 'orden(es) esperando el costo real de Stripe' : 'order(s) waiting for the actual Stripe fee'}` : (lang === 'es' ? 'Diferencia sobre la tarifa estándar de tarjeta' : 'Difference above the standard card fee')}</p></div>
          <div className="financial-tone-emerald rounded-2xl border border-emerald-400/35 bg-emerald-500/15 p-4"><p className="financial-card-label text-xs font-bold uppercase tracking-wide text-emerald-300">{lang === 'es' ? 'Pagado al organizador' : 'Paid to organizer'}</p><p className="financial-card-value mt-2 text-2xl font-black text-emerald-300">{money(summary.organizerPaid)}</p><p className="financial-card-note mt-1 text-xs font-semibold text-emerald-100/65">{detail.organizerPayouts.length} {lang === 'es' ? 'pago(s) registrado(s)' : 'recorded payout(s)'}</p></div>
          <div className="financial-tone-red rounded-2xl border border-red-400/35 bg-red-500/15 p-4"><p className="financial-card-label text-xs font-bold uppercase tracking-wide text-red-300">{lang === 'es' ? 'Pendiente por pagar' : 'Pending payout'}</p><p className="financial-card-value mt-2 text-2xl font-black text-red-300">{money(summary.organizerPending)}</p><p className="financial-card-note mt-1 text-xs font-semibold text-red-100/65">{lang === 'es' ? 'Saldo que aún debes transferir' : 'Balance still to transfer'}</p></div>
        </div>
        {summary.pendingFeeReconciliations > 0 && <div className="mx-4 mb-4 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm font-semibold text-amber-800">{lang === 'es' ? 'El pago al organizador queda protegido hasta que Stripe confirme el costo real de las órdenes Klarna pendientes. Las ventas y las entradas no se detienen.' : 'Organizer payout is protected until Stripe confirms the actual cost of pending Klarna orders. Sales and ticket issuance continue.'}</div>}
