    ...
    <div className="premium-shell space-y-6 p-6 lg:p-8">
      ...
      <section className="overflow-hidden rounded-2xl border border-[rgba(10,55,90,0.10)] bg-white shadow-sm">
        ...
        <div className="grid grid-cols-1 gap-3 p-4 md:grid-cols-2 xl:grid-cols-4">
          ...
          <div className="rounded-2xl border border-orange-200 bg-orange-50/70 p-4"><p className="text-xs font-bold uppercase tracking-wide text-[#C65A09]">{lang === 'es' ? 'Ajuste adicional de Klarna' : 'Additional Klarna adjustment'}</p><p className="mt-2 text-2xl font-black text-[#0A375A]">-{money(summary.organizerProcessingAdjustments)}</p><p className="mt-1 text-xs font-semibold text-gray-500">{summary.pendingFeeReconciliations > 0 ? `${summary.pendingFeeReconciliations} ${lang === 'es' ? 'orden(es) esperando el costo real de Stripe' : 'order(s) waiting for the actual Stripe fee'}` : (lang === 'es' ? 'Diferencia sobre la tarifa estándar de tarjeta' : 'Difference above the standard card fee')}</p></div>
          <div className="rounded-2xl border border-emerald-400/45 bg-emerald-950/85 p-4 shadow-sm shadow-emerald-950/30"><p className="text-xs font-bold uppercase tracking-wide text-emerald-200">{lang === 'es' ? 'Pagado al organizador' : 'Paid to organizer'}</p><p className="mt-2 text-2xl font-black text-white">{money(summary.organizerPaid)}</p><p className="mt-1 text-xs font-semibold text-emerald-100">{detail.organizerPayouts.length} {lang === 'es' ? 'pago(s) registrado(s)' : 'recorded payout(s)'}</p></div>
          <div className="rounded-2xl border border-red-400/45 bg-red-950/85 p-4 shadow-sm shadow-red-950/30"><p className="text-xs font-bold uppercase tracking-wide text-red-200">{lang === 'es' ? 'Pendiente por pagar' : 'Pending payout'}</p><p className="mt-2 text-2xl font-black text-white">{money(summary.organizerPending)}</p><p className="mt-1 text-xs font-semibold text-red-100">{lang === 'es' ? 'Saldo que aún debes transferir' : 'Balance still to transfer'}</p></div>
        </div>
