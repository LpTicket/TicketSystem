...
export default function AdminEventFinancialDetailPage() {
  ...
  return (
    <div className="premium-shell space-y-6 p-6 lg:p-8">
      ...
      {discrepancy && <div className="flex gap-3 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-amber-900"><HiOutlineExclamationCircle className="mt-0.5 h-5 w-5 shrink-0" /><div><p className="font-black">{lang === 'es' ? 'Diferencia entre órdenes y entradas emitidas' : 'Difference between orders and issued tickets'}</p><p className="mt-1 text-sm font-medium">{summary.extraIssuedTickets > 0 ? `${summary.extraIssuedTickets} ${lang === 'es' ? 'entrada(s) adicional(es) emitida(s).' : 'extra ticket(s) issued.'}` : `${summary.missingTickets} ${lang === 'es' ? 'entrada(s) pendiente(s) de emitir.' : 'ticket(s) still missing.'}`} {lang === 'es' ? 'Los ingresos se calculan desde las órdenes pagadas, no desde estas entradas.' : 'Revenue is calculated from paid orders, not from these tickets.'}</p></div></div>}
