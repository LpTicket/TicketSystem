      ...
      <form onSubmit={handleCreate} className="public-premium-card p-5 md:p-6 space-y-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">{lang === 'es' ? 'La recompensa se configura en cada evento y se paga manualmente.' : 'Reward is configured per event and paid manually.'}</p>
          </div>
        </div>
      ...
      <div className="public-premium-card overflow-hidden">
        <div className="p-5 border-b border-gray-100 flex items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-black text-gray-900">{lang === 'es' ? 'Recompensas por evento' : 'Event rewards'}</h2>
              <p className="text-sm text-gray-500">
                {lang === 'es'
                  ? 'Administra la recompensa base por evento cuando se vende con códigos de creador.'
                  : 'Manage the base reward per event when creator codes are used.'}
              </p>
            </div>
          </div>
        </div>
                ...
                <div key={event.id} className="p-5 grid grid-cols-1 lg:grid-cols-[1.4fr_1fr_0.7fr_0.9fr_auto] gap-4 items-center">
                  ...
                  <label className="space-y-1 block">
                    <span className="text-xs font-black uppercase tracking-[0.16em] text-gray-400">
                      {lang === 'es' ? 'Recompensa base ($)' : 'Base reward ($)'}
                    </span>
                    <input
                      id={inputId}
                      type="number"
                  ...
                  <button
                    ...
                    onClick={async () => {
                      ...
                      try {
                        ...
                        toast.success(lang === 'es' ? 'Recompensa actualizada' : 'Reward updated');
                      } catch (error: any) {
                        toast.error(error?.response?.data?.message || (lang === 'es' ? 'No se pudo actualizar la recompensa' : 'Could not update reward'));
                      }
                    }}
        ...
        <div className="p-5 border-b border-gray-100 flex items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500">
                {lang === 'es'
                  ? 'Reporte real de entradas vendidas y recompensas generadas por cada código.'
                  : 'Real report of tickets sold and rewards generated by each code.'}
              </p>
            </div>
          </div>
        </div>
            ...
            <table className="w-full min-w-[900px] text-sm">
              <thead className="bg-gray-50 text-xs uppercase tracking-[0.16em] text-gray-400">
                <tr>
                  ...
                  <th className="px-5 py-3 text-right">{lang === 'es' ? 'Recompensa' : 'Reward'}</th>
