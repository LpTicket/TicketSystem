...
| Marketing email, SMS, WhatsApp y push | IMPLEMENTADO, NO PROBADO | Las campañas nuevas usan Zoho Campaigns, no el SMTP normal de Zoho Mail. El backend crea una lista privada por intento, con identificador único, la vincula explícitamente al tema de consentimiento requerido por Zoho, entrega el mismo diseño premium por URL y envía la campaña completa sin lotes manuales de 100; guarda destinatarios y estados persistentes por campaña. La creación de listas y carga en lote usan el parámetro oficial `emailids` (máximo diez correos por solicitud), solicitan el formato `JSON` y reconocen las claves `listkey` y `campaignKey` del proveedor. La autorización OAuth cifra su token en PostgreSQL con una clave derivada de `JWT_SECRET`; Client ID y Client Secret permanecen como variables privadas de Railway. Si Zoho rechaza la conexión, el panel ofrece una reconexión de consentimiento firmada y de corta duración, sin exponer secretos. El tema de consentimiento `LPTicket promociones y novedades` quedó creado en Zoho y configurado como variable privada de Railway. Las respuestas no exitosas de Zoho se conservan con su código y mensaje real, sin marcar ni duplicar destinatarios. El panel mantiene hasta 50 campañas, permite abrir cada análisis individual y eliminar una campaña completada o pausada con confirmación. Pendiente prueba real de aceptación y métricas finales del proveedor. |
