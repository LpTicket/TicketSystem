...
| Escáner de empleados | IMPLEMENTADO Y COMPROBADO LOCALMENTE | Solicitudes, aprobación, rechazo, revocación, búsqueda y validación presentes. El administrador cuenta con el acceso directo `Empleados de eventos` dentro de su panel web y puede consultar y decidir solicitudes de cualquier evento, separadas por evento e incluyendo los estados pendiente, aprobado, rechazado y revocado. |
...
| Panel administrador | IMPLEMENTADO, NO PROBADO | Usuarios, eventos, facturas, marketing, categorías y analítica presentes. Incluye ahora el acceso directo `Empleados de eventos` para operar los permisos de scan de cualquier evento sin salir del panel administrativo. El dashboard y Analíticas muestran compras Klarna, compradores recientes, total cobrado, ajuste adicional del organizador y conciliaciones pendientes. Los alias SQL reservados de `/api/admin/stats` y `/api/analytics/summary` fueron corregidos; el segundo está comprobado localmente y pendiente de publicación. Web y móvil ya no ocultan una falla de Analíticas: muestran un mensaje y permiten reintentar. Localmente, el desglose financiero usa superficies oscuras translúcidas y colores semánticos: Stripe morado, ganancia verde, Klarna rojo/rosa, ajuste Klarna ámbar y pendiente rojo. Etiquetas, cifras y notas usan selectores semánticos propios para conservar el tono vivo de cada tarjeta frente a la regla global gris del panel. El resumen por evento diferencia también el pago al organizador en verde, el ajuste en naranja y lo pendiente en rojo. Pendiente comprobar visualmente esta actualización de colores y Analíticas en producción. |
