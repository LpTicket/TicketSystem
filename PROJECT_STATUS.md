...
- El desglose nuevo de Klarna en dashboard y Analíticas está implementado. Se identificó y corrigió localmente el error SQL que dejaba `/admin` vacío en producción; la corrección todavía requiere publicación y comprobación.
...
| Panel administrador | IMPLEMENTADO, NO PROBADO | Usuarios, eventos, facturas, marketing, categorías y analítica presentes. El dashboard y Analíticas muestran compras Klarna, compradores recientes, total cobrado, ajuste adicional del organizador y conciliaciones pendientes. Los registros de producción confirmaron que `/api/admin/stats` fallaba por usar `order` como alias SQL reservado; el alias fue corregido localmente y el frontend ahora muestra un error con reintento en vez de quedar vacío. El resumen financiero por evento muestra además el monto ya pagado y el saldo exacto pendiente del organizador con las tres tarjetas operativas en naranja oscuro translúcido. Pendiente publicar y comprobar visualmente en producción. |
