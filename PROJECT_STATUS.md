...
| Marketing email, SMS, WhatsApp y push | IMPLEMENTADO, NO PROBADO | Código presente; entrega depende de proveedores externos. Cada campaña de email guarda sus destinatarios y estados individuales antes de enviar; inicia un lote de hasta 100 sin bloquear el navegador, muestra enviados, pendientes, rechazos SMTP y aperturas aproximadas, y permite continuar el siguiente lote sin repetir los enviados. El panel mantiene hasta 50 campañas, permite abrir cada análisis individual y eliminar una campaña completada o pausada con confirmación; no permite borrar una campaña que todavía esté enviando. Muestra un error visible si el backend no responde, en vez de ocultar los datos. Puede registrar el envío histórico de Zoho sin SMTP ni cambios de esquema. `Enviado` significa aceptado por Zoho SMTP; la entrega final y rebotes tardíos requieren eventos del proveedor. Los emails incluyen una acción secundaria hacia la app oficial de LPTicket en App Store; pendiente prueba real. |
