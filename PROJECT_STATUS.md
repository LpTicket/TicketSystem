...
| Marketing email, SMS, WhatsApp y push | IMPLEMENTADO, NO PROBADO | Las campañas nuevas usan Zoho Campaigns, no el SMTP normal de Zoho Mail. Antes de llamar a `sendcampaign`, el backend consulta el borrador y verifica que Zoho haya asociado la lista privada como audiencia; si no, pausa sin enviar correos. Para cuentas con Topics, cada destinatario se suscribe mediante la API oficial `listsubscribe` con el `topic_id` configurado antes de crear el borrador. Reutiliza listas privadas de la misma audiencia para no duplicarlas. El access token de Zoho se reutiliza durante su vigencia y las llamadas simultáneas comparten una sola renovación, evitando el límite oficial de diez tokens en diez minutos que causaba el ciclo falso de reconexión. La autorización OAuth cifra su refresh token en PostgreSQL con una clave derivada de `JWT_SECRET`; Client ID y Client Secret permanecen como variables privadas de Railway. Las respuestas no exitosas se conservan con su código y mensaje real. El panel mantiene hasta 50 campañas y permite abrir o eliminar su análisis. Pendiente prueba real de aceptación, envío y métricas finales del proveedor. |
