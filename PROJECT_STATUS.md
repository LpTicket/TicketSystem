...
| Marketing email, SMS, WhatsApp y push | IMPLEMENTADO, NO PROBADO | Las campañas nuevas usan Zoho Campaigns, no el SMTP normal de Zoho Mail. Antes de llamar a `sendcampaign`, el backend consulta el borrador y verifica que Zoho haya asociado la lista privada como audiencia; si no, pausa sin enviar correos. Para cuentas con Topics, cada destinatario se suscribe mediante la API oficial `listsubscribe` con el `topic_id` configurado antes de crear el borrador; así se evita el error `6606` de listas sin contactos clasificados. Reutiliza una lista privada previa de la misma audiencia cuando Zoho conservó un intento fallido, para no crear listas duplicadas en cada reintento. La autorización OAuth cifra su token en PostgreSQL con una clave derivada de `JWT_SECRET`; Client ID y Client Secret permanecen como variables privadas de Railway. La próxima reconexión solicita permisos de lectura, creación y actualización para comprobar listas y borradores sin exponer secretos. Las respuestas no exitosas de Zoho se conservan con su código y mensaje real, sin marcar ni duplicar destinatarios. El panel mantiene hasta 50 campañas, permite abrir cada análisis individual y eliminar una campaña completada o pausada con confirmación. Pendiente prueba real de aceptación y métricas finales del proveedor. |
